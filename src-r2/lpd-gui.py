...
def save_arguments_to_file(csv_file, kva_value, datetime_value):
    ...
    try:
        ...
        with open(args_file, "w") as file:
            ...
            file.write("\n")
            update_status(f"Arguments.txt file written.", "sucess")
    except Exception as e:
...
def launch_analysis(csv_file, kva_value, datetime_value=None):
    ...
    try:
        ...
        update_status("Arguments.txt file written.", "success")
        ...
        update_status("Running lpd-main.py to create _RESULTS.txt file.", "info")

        # Run weather analysis
        weather_file = f"{base}_WEATHER.csv"
        launch_weather_analysis()
        update_status(f"Reading _WEATHER.csv file and launching weather analysis.", "info")

        load_profile_file = f"{base}_RESULTS-LP.csv"
        merge_command = [sys.executable, "lpd-merge.py"] + base_command
        subprocess.run(merge_command, check=True)
        update_status(f"Merging with _WEATHER.csv with _RESULTS-LP.csv.", "info")
        ...
        update_status("Launching load profile visualzation.", "info")
        ...
        if not os.path.isfile(results_file):
            ...
            update_status(f"Error: _RESULTS.txt not generated by lpd-main.py.", "error")
            return
        ...
        update_status(f"A subprocess error occured in launch_analysis().", "error")
    except Exception as e:
        print(f"An error occurred: {e}")
        update_status(f"An error occured in launch_analysis().", "error")
